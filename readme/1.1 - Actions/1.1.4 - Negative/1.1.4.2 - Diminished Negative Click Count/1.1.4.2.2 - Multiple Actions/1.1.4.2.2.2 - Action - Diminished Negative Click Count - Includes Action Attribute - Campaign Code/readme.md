In this example we will get the `Diminished Negative Click Count` for all `Actions` whose name includes "Usecase - Popup".
...
As we are aggrigating, we first need to select which aggrigation `function` we are using. We will be using `Sum of the elements in` to get the value of `Diminished Negative Click Count` for each matching `Action`:
