In this example, we will get the sum of `Activity Counts` where the `Proposition` is the children of the `Proposition` "Level 2" but not itself, and the `Activity Type` is "AT 1".
