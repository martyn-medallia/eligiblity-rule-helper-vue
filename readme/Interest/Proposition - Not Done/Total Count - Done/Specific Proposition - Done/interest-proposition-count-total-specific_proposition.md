In this example, we will get the `Total Count` where the `Proposition` is "Root". This shows us how to get the `Total Count` for any case where we are exactly specifying the `Proposition`.
